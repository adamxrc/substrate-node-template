...
pub mod pallet {
	...
	const HTTP_HEADER_USER_AGENT: &str = "adamxrc";
	...
	#[derive(Deserialize, Encode, Decode, Default)]
	struct AssetInfo {
		#[serde(rename(deserialize = "priceUsd"), deserialize_with = "de_string_to_bytes")]
		price_usd: Vec<u8>,
	}
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		...
		fn offchain_worker(block_number: T::BlockNumber) {
			...
			let result = match modu {
				...
				1 => Self::fetch_price_info(),
				2 => Self::fetch_price_info(),
				3 => Self::fetch_price_info(),*/
	...
	impl<T: Config> Pallet<T> {
			...
			_signature: T::Signature) -> DispatchResult
		{
			...
			// we don't need to verify the signature here because it has been verified in
			//   `validate_unsigned` function when sending out the unsigned tx.
			let PricePayload { price_int, price_frac, public } = payload;
			log::info!("submit_price_unsigned_with_signed_payload: ({}, {:?}, {:?})", price_int, price_frac, public);
			Self::add_price(price_int, price_frac);

			Self::deposit_event(Event::NewPrice(price_int, price_frac));
			Ok(())
		}
	}
	...
	impl<T: Config> Pallet<T> {
		...
		fn add_price(price_int: u64, price_frac: Permill) {
			Prices::<T>::mutate(|prices| {
				if prices.len() == NUM_VEC_LEN {
					let _ = prices.pop_front();
				}
				prices.push_back((price_int, price_frac));
		...
		fn fetch_price_and_signed() -> Result<(), Error<T>> {
			...
			let price: Vec<u8> = Self::fetch_price()?;

			let price_str = str::from_utf8(&price).map_err(|_| <Error<T>>::HttpFetchingError)?;

			let price_int = price_str.parse::<f64>().unwrap() as u64;

			let idx = Self::first_dot(&price_str);

			let price_frac: Permill = Permill::from_parts(price_str[idx+1..idx+6].parse::<u32>().unwrap());

			if let Some((_, res)) = signer.send_unsigned_transaction(
				|acct| PricePayload { price_int, price_frac, public: acct.public.clone() },
		...
		fn fetch_from_remote(remote_url: &str) -> Result<Vec<u8>, Error<T>> {
			log::info!("sending request to: {}", remote_url);
